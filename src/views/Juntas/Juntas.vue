  	...
  	<v-row justify="center">
  		<v-col cols="12" > 
            ...
            ></v-select>
		    	...
		    	<v-card class="pa-5">
		    		<controlJuntas :modoVista="modoVista" :parametros="parametros" @modal="dialog = $event" @recarga="recarga = $event"/>
		    	</v-card>
		    </v-dialog>
  		</v-col>
  	</v-row>
...
<script>
	import controlJuntas from '@/views/Juntas/controlJuntas.vue'
  ...
  moment.locale('es');
	...
	export default {
		...
		data () {
				return {
					titulo: 'Programación sala de juntas',
					recarga: 0,
					page: 0,
					pageCount: 0,
					itemsPerPage: 100,
					...
					parametros:'',
					headers:[
            { text: 'QUIEN RESERVA', align: 'left'  , value: 'nomusuario'	 },
						{ text: 'CONCEPTO'     , align: 'left'  , value: 'concepto'  },
						{ text: 'FECHA'        , align: 'left'  , value: 'fecha'   },
						{ text: 'HORA INICIO'     		 , align: 'left'  , value: 'hora' },
						{ text: 'HORA FIN'     , align: 'left'  , value: 'hora2'   },
			...
			methods:{
				...
				ImprimirExcel(){
					...
					let tInformacion = this.getJuntas
