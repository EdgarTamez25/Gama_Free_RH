...
<script>
  ...
  moment.locale('es');
	...
	export default {
		...
		data () {
				return {
					titulo:'ENTRADAS/SALIDAS EMPLEADOS',
					page: 0,
					pageCount: 0,
					itemsPerPage: 100,
					search: '',
            ...
            { text: 'Empleado'     , align: 'left'  , value: 'nombre' },
						...
						{ text: 'Fecha Salida' , align: 'left'  , value: 'fecha1' },
						{ text: 'Fecha Entrada', align: 'left'  , value: 'fecha2' },
						{ text: 'Hora Salida'  , align: 'left'  , value: 'hora1' },
						{ text: 'Hora Entrada' , align: 'left'  , value: 'hora2' },
			...
			computed:{
				...mapGetters('Accesos' ,['Loading','getAccesosAll']), 
				...mapGetters('Login',['getdatosUsuario']),
			},

			methods:{
			...mapActions('Accesos',['consultarAccesosAll']),
			

				init(){
					const payload = { id_sucursal: this.sucursal.id ,fecha1: this.date , fecha2: this.date2}
					this.consultarAccesosAll(payload) 
				},
				...
				ImprimirExcel(){
					...
					let tInformacion = this.getAccesosAll
					this.manejarDescarga(this.titulo + ' '+this.sucursal.nombre ,tHeaders,tValores,tInformacion)
				},
				...
				// llenarCampos(){
				// 	this.sucursal = { id: this.getdatosUsuario.id_sucursal, 
				// 	                  nombre: this.sucursales[this.getdatosUsuario.id_sucursal-1].nombre}
				// }
			}
	}
</script>
